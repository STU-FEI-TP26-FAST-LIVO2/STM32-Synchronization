...
Tento projekt predstavuje aplikáciu  STM32 navrhnutú na emuláciu a synchronizáciu:
...
Firmvér je implementovaný na mikrokontroléri (MCU) STM32F103 s využitím ovládačov STM32 HAL vygenerovaných nástrojom STM32CubeMX.
...
## Postup obnovy (Recovery)

V prípade zlyhania synchronizácie alebo neočakávaného správania počas behu programu **reštartujte zariadenie STM32**.
...
Vygenerované pomocou nástrojov:
...
Kontrolný súčet (checksum) sa vypočítava dynamicky pred každým odoslaním.

### Tok programu pri prenose GPS

1. Nastane udalosť časovača
2. Interné softvérové hodiny sa inkrementujú (zvýšia o 1)
3. Firmvér vygeneruje správu GPRMC
...
* Meranie teploty
* Sekvenčné čítanie surových dát (raw burst reading)
...
Pin IMU senzora indikujúci pripravenosť dát (data-ready) spúšťa prerušenie EXTI.
...
* Prenosová rýchlosť (Baudrate): 9600
* DMA prenos
...
* Prenosová rýchlosť (Baudrate): 115200
...
Pre hodnotu ARR (Auto-Reload Register) sme zvolili hodnotu blízku maximu 16-bitového celého čísla bez znamienka (unsigned), aby sme dosiahli maximálne rozlíšenie časovej značky.
...
Strieda PWM (Duty cycle) (PWM znamená ŠIM):
...
Preddelička časovača (Prescaler):
...
## Perióda pretečenia TIM1 (Overflow Period)
...
* Zníženie zaťaženia procesora v hlavnej slučke (main loop)
...
* Udalosťami riadenú (event-driven) synchronizáciu
...
## Úlohy hlavnej slučky (Main Loop)
...
* Riadená prerušeniami (interrupt-driven)
* Podporovaná DMA prenosmi
* Neblokujúca
...
Zachytenie presného času
        ↓
Spustenie sekvenčného čítania (burst read) cez I2C
        ↓
Odoslanie dát z IMU cez UART2
```

### Hardvérové spúšťanie kamery

```text
PWM výstup z TIM2
        ↓
Generovanie spúšťacieho signálu s frekvenciou 10 Hz
        ↓
...
Implementácia využíva efektívne techniky zabudovaných (embedded) systémov:

* DMA
* Prerušenia
* Časové pečiatkovanie (timestamping)
* Asynchrónne prenosy
* PWM synchronizáciu

Vďaka tomu je tento firmvér vhodný pre aplikácie vyžadujúce synchronizáciu senzorov v reálnom čase.